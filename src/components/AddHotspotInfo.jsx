...
function AddHotspotInfo({ title, type }) {
    ...
    return (
        <div>
            <p>Add new <b>{type}</b></p>
            <button onClick={getOrientationHandler}>get orientation</button>
            <p>y: <b>{yaw}</b> p: <b>{pitch}</b></p>

            <form onSubmit={submitHandler}>
                <label htmlFor="infoHotspot">text</label>
                <input type="text" name='infoHotspot' value={info} onChange={(e) => setInfo(e.target.value)} required />
